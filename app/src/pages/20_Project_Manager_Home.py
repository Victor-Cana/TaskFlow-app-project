...
if st.button('Create a Report', 
  ...
  st.switch_page('pages/23_Create_Report.py')

if st.button('Grant Resource Access', 
             type='primary',
             use_container_width=True):
  st.switch_page('pages/24_Grant_Resource_Access.py')

if st.button('Revoke Resource Access', 
             type='primary',
             use_container_width=True):
  st.switch_page('pages/25_Revoke_Resource_Access.py')

if st.button('Assign User to Project', 
             type='primary',
             use_container_width=True):
  st.switch_page('pages/26_Assign_User_to_Project.py')

if st.button('Remove User from Project', 
             type='primary',
             use_container_width=True):
  st.switch_page('pages/27_Remove_User_from_Project.py')

if st.button('Send Project Message', 
             type='primary',
             use_container_width=True):
  st.switch_page('pages/28_Send_Project_Message.py')

if st.button('Update Message', 
             type='primary',
             use_container_width=True):
  st.switch_page('pages/29_Update_Message.py')

if st.button('View Project Resources', 
             type='primary',
             use_container_width=True):
  st.switch_page('pages/30_View_Project_Resources.py')

if st.button('View Resource Work Duration', 
             type='primary',
             use_container_width=True):
  st.switch_page('pages/31_View_Resource_Work_Duration.py')